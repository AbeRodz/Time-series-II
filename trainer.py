...
class Trainer():
	...
	def train_one_epoch_time_series(
		self,
		use_amp=False,
		dtype=torch.bfloat16
	):
		"""
		Entrenamiento de un epoch para series de tiempo.

		Args:
			use_amp (bool): Activar Automatic Mixed Precision (AMP). Default: False
			dtype (torch.dtype): Precision de AMP. Default: torch.bfloat16

		Returns:
			float: Promedio de los últimos 10 valores de loss.
		"""
		model = self.model.train()
		scaler = torch.amp.GradScaler(enabled=use_amp)
		losses = []
		...
		for train_input, train_target, _dates in bar:  # Ahora train_target es tu valor real

			train_input = train_input.to(self.device, non_blocking=True)  # [B, T, features]
			train_target = train_target.to(self.device, non_blocking=True)  # [B, 1] o [B, T, features]

			with torch.autocast(device_type=self.device, dtype=dtype, enabled=use_amp):
				output = model(train_input)  # [B, 1] si predices un paso
				loss = self.loss_fn(output, train_target)
		...
		# Último paso si no coincide con gradient accumulation
	...
	def train_model_v2(self, use_amp: bool = False, dtype: torch.dtype = torch.bfloat16):
		...
		for train_input, train_mask in bar:
			...
			with torch.autocast(device_type=self.device, dtype=dtype, enabled=use_amp):
				...
				B, T = output.shape
				loss = self.loss_fn(output.view(B * T), train_mask.view(B * T))
			loss_for_logging = loss.detach()
			loss = loss / self.gradient_accumulation_steps
